...
const AdminBlogs = () => {
  ...
  const DeleteBlog = async (blogId) => { // Renamed for clarity
    try {
      const response = await axios.delete(`http://localhost:4000/api/user/deleteblog/${blogId}`);
    ...
    <div className="">
      ...
      <Paper sx={{ width: '100%', overflowX:'scroll' }}>
        <TableContainer sx={{ maxHeight: 440 }}>
          <Table stickyHeader aria-label="sticky table"  style={{ maxWidth: '100%', overflowX: 'Auto' }}>
            <TableHead>
              <TableRow>
                {/* {Blogs.map((blog) => (
            ...
            <TableBody>
              ...
              {Blogs.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((blog, index) => (
                <TableRow key={blog.id} className="border">
                  ...
                  <TableCell onClick={() => DeleteBlog(blog._id)}><MdDeleteOutline/></TableCell>
